<?php
    ...
    $url = "https://igrejaemanuel.com.br/carteirinha?m=$matricula";
    ...
    */
    ...
    if(empty($matricula)):
        $matricula = $matricula; //Se não for o campo da empresa utilizamos este texto.
    endif;

    //Gerar a imagem e salvar a imagem do QR no servidor
    @mkdir("arq/", 0777); //Cria a pasta se não houver. o @ é pra ocultar erro caso
    @mkdir("arq/membros/", 0777); //Cria a pasta se não houver. o @ é pra ocultar erro caso
    @mkdir("arq/membros/$matricula/", 0777); //Cria a pasta se não houver. o @ é pra ocultar erro caso
    @mkdir("arq/membros/$matricula/qrcode/", 0777); //Cria a pasta se não houver. o @ é pra ocultar erro caso

    //Gerar a imagem e salvar a imagem do QR no servidor
    $qrcode->render($url, "arq/membros/$matricula/qrcode/$matricula.png");

    $PastaDoQrCode = "arq/membros/$matricula/qrcode/$matricula.png";
