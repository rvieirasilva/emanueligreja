...
<?php
	...
	if(isset($_POST['btn-finalizar'])):
		$ctg = $_POST['categoria'];
		//Inserir mais de uma categoria.
		for($a = 0; $a < count($_POST['categoria']); $a++){
			$referencia         = mt_rand();
			...
			// $categoria          = mysqli_escape_string($con, $_POST['categoria'][$a]);
			$categoria          = "Live";
				//if(empty($categoria)): $categoria = 'Mensagens'; endif;
			...
			for($imgmini = 0; $imgmini < $contagemdasminiaturas; $imgmini++) {
				if(!empty($_FILES['miniatura']['name'][$imgmini])):
					...
					if($contagemdasminiaturas <= 1):
						$inserirminiaturas= "INSERT INTO blog (qrcode, referencia, titulo, texto, audio, video, textoancora, categoria, autor, cargo, datadapostagem, horadapostagem, anexo, publico, ano, mes, miniatura, capa, bibliografia, visualizacoes) VALUES ('', '$referencia', '$titulo', '$texto', '$audio', '$upvideo', '$textoancora', '$categoria', '$nomedocolaborador', '$cargo', '$datadapostagemHora', '$horadapostagem', '$anexo', '', '$ano', '$mes', '$miniatura', '$capa', '$bibliografia', '')";
					elseif($contagemdasminiaturas > 1):
						$inserirminiaturas= "INSERT INTO blog (qrcode, referencia, titulo, texto, audio, video, textoancora, categoria, autor, cargo, datadapostagem, horadapostagem, anexo, publico, ano, mes, miniatura, capa, bibliografia, visualizacoes) VALUES ('', '$referencia', '$titulo', '', '', '', '', '$categoria', '$nomedocolaborador', '$cargo', '$datadapostagemHora', '$horadapostagem', '', '', '$ano', '$mes', '$miniatura', '$capa', '', '')";
					endif;
					...
					if(mysqli_query($con, $inserirminiaturas)):
						...
						$mensagem = ("\n$nomedocolaborador ($matricula) Postou o texto $titulo. Isto através do IP: $ip, na data $dia às $hora.\n"); //Escreva a mensagem que será gravada no log.
					...
					$inserir= "INSERT INTO blog (qrcode, referencia, titulo, texto, audio, video, textoancora, categoria, autor, cargo, datadapostagem, hora, anexo, publico, ano, mes, miniatura, capa, bibliografia, visualizacoes) VALUES ('', '$referencia', '$titulo', '$texto', '$audio', '$upvideo', '$textoancora', '$categoria', '$nomedocolaborador', '$datadapostagemHora', '$horadapostagem', '$anexo', '', '$ano', '$mes', '$miniatura', '$capa', '$bibliografia', '')";
                                        ...
                                        <div class="title-container-inner">
                                            <div class="row flex">
                                                <div class="column width-8 v-align-middle">
                                                    <div>
                                                        <h1 class="mb-0">Programar <strong>live</strong></h1>
