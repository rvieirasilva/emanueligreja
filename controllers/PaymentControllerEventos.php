<?php
    ...
    $codigodainscricao                              = filter_input(INPUT_POST,'codigodainscricao',FILTER_DEFAULT);
    ...
    if(!empty($ErroDoPagamento)):
        ...
        if($status_Do_Pagamento == 'APROVADO' OR $status_Do_Pagamento == 'approved'):
            ...
            if(mysqli_query($con, $inFin)):
                $upEvento="UPDATE agendainscritos SET statusdopagamento='$status_Do_Pagamento', formadepagamento='$formadepagamento', parcelas='$installments' WHERE codigodainscricao='$codigodainscricao'";
                mysqli_query($con, $upEvento);
                ...
                $mail->addBCC("financas@igrejaemanuel.com.br");	//Cópia oculta do e-mail
                    ...
                    //$mail->addAttachment("/tmp/image.jpg", "new.jpg");    // Optional name
                $mail->isHTML(true);                                  // Set email format to HTML
                $mail->Subject = "Igreja Emanuel - O pagamento da sua inscrição foi realizado";
        ...
        elseif($status_Do_Pagamento == 'PENDENTE' OR $status_Do_Pagamento == 'in_process'):
            ...
            $mail->addBCC("financas@igrejaemanuel.com.br");	//Cópia oculta do e-mail
                ...
                //$mail->addAttachment("/tmp/image.jpg", "new.jpg");    // Optional name
            $mail->isHTML(true);                                  // Set email format to HTML
            $mail->Subject = "Igreja Emanuel - Pagamento pendente";
            $mail->Body    = "Ficamos contentes com sua inscrição no evento, contudo, precisamos informar que a administradora retornou que seu pagamento está pendente. Ela pode ser processada em até dois dias ou não. Entre em contato com a administradora do seu cartão e com nossa equipe que te auxiliaremos.
                            <h3><strong>Igreja Emanuel</strong></h3>
                            <p></p>
                            ";
            $mail->AltBody    = "Ficamos contentes com sua inscrição no evento, contudo, precisamos informar que a administradora retornou que seu pagamento está pendente. Ela pode ser processada em até dois dias ou não. Entre em contato com a administradora do seu cartão e com nossa equipe que te auxiliaremos.
                            <h3><strong>Igreja Emanuel</strong></h3>
                            <p></p>
                            "; // Para o trello!
            $mail->send();
